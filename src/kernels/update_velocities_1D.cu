#ifndef FIBERS_UPDATE_VELOCITIES_1D_KERNEL_
#define FIBERS_UPDATE_VELOCITIES_1D_KERNEL_

#include "constants.cu"

__device__
    void compute_GV_1D(const int j,
...
__global__ void update_velocities_1D(
...
)
{
  ...
  translational_velocities[i].x = 0.0;
  translational_velocities[i].y = 0.0;
  translational_velocities[i].z = 0.0;
  ...
  for (int j = 0; j < NUMBER_OF_FIBERS; ++j)
  {
    ...
    compute_GV_1D(j, position_i, orientation_i, position_j, orientation_j, coefficients, external_force, GF);
...
#endif //FIBERS_UPDATE_VELOCITIES_1D_KERNEL_
